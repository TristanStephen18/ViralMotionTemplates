...
const KenBurnsMedia: React.FC<{
  ...
  objectFit?: "cover" | "contain" | "fill";
  eraserMask?: string;
}> = ({ src, type, config, progress, filter, adjustments, flipX, flipY, objectFit, eraserMask }) => {
  const scale = interpolate(progress, [0, 1], [config.zoomStart, config.zoomEnd]);
  const tx = interpolate(progress, [0, 1], [0, Math.sign(config.panX) * Math.min(Math.abs(config.panX) * 0.15, 16)]);
  const ty = interpolate(progress, [0, 1], [0, Math.sign(config.panY) * Math.min(Math.abs(config.panY) * 0.15, 16)]);

  const computedFilter = buildAdjustmentFilter(adjustments, filter);
  const finalFilter = computedFilter !== 'none' ? computedFilter : "contrast(1.07) saturate(1.08) brightness(1.02)";
  const flipTransform = `${flipX ? 'scaleX(-1)' : ''} ${flipY ? 'scaleY(-1)' : ''}`.trim();
    ...
    <AbsoluteFill style={{ overflow: "hidden", background: "#000" }}>
      <div
        style={{
          ...
          ...(eraserMask ? {
            WebkitMaskImage: `url(${eraserMask})`,
            maskImage: `url(${eraserMask})`,
      ...
      >
        ...
        <div
          style={{
            position: "absolute",
            inset: 0,
            background: "radial-gradient(80% 80% at 50% 50%, rgba(0,0,0,0) 50%, rgba(0,0,0,0.35) 100%)",
          }}
        />
      </div>
    </AbsoluteFill>
...
progress={kbProgress}
  stageW={cardWidth}
  stageH={cardHeight}
  filter={currentLayer.type === 'image' ? (currentLayer as ImageLayer).filter : (currentLayer as VideoLayer).filter}
adjustments={currentLayer.type === 'image' ? (currentLayer as ImageLayer).adjustments : (currentLayer as VideoLayer).adjustments}
flipX={currentLayer.type === 'image' ? (currentLayer as ImageLayer).flipX : (currentLayer as VideoLayer).flipX}
flipY={currentLayer.type === 'image' ? (currentLayer as ImageLayer).flipY : (currentLayer as VideoLayer).flipY}
objectFit={currentLayer.type === 'image' ? (currentLayer as ImageLayer).objectFit : (currentLayer as VideoLayer).objectFit}
eraserMask={currentLayer.type === 'image' ? (currentLayer as ImageLayer).eraserMask : undefined}
/>
        </AbsoluteFill>
  ...
  stageW={cardWidth}
  stageH={cardHeight}
  filter={nextLayer.type === 'image' ? (nextLayer as ImageLayer).filter : (nextLayer as VideoLayer).filter}
  adjustments={nextLayer.type === 'image' ? (nextLayer as ImageLayer).adjustments : (nextLayer as VideoLayer).adjustments}
  flipX={nextLayer.type === 'image' ? (nextLayer as ImageLayer).flipX : (nextLayer as VideoLayer).flipX}
  flipY={nextLayer.type === 'image' ? (nextLayer as ImageLayer).flipY : (nextLayer as VideoLayer).flipY}
  objectFit={nextLayer.type === 'image' ? (nextLayer as ImageLayer).objectFit : (nextLayer as VideoLayer).objectFit}
  eraserMask={nextLayer.type === 'image' ? (nextLayer as ImageLayer).eraserMask : undefined}
/>
          </AbsoluteFill>
        )}
      </div>
...
const ChatBubbleComponent: React.FC<{
  ...
  const isPhoneStyle = layer.chatStyle !== "fakechatconversation";
  ...
  const containerStyle: React.CSSProperties = {
    ...
    transform: `translate(${isPhoneStyle ? (layer.isSender ? -100 : -5) : (layer.isSender ? -50 : -70)}%, -50%) rotate(${rotation}deg) ${entrance.transform}`,
      ...
      >
        ...
        <div
          style={{
            ...
            fontSize: fontSize,
            lineHeight: 1.6,
            fontFamily: resolveFontFamily(layer.fontFamily as string) || resolveFontFamily('Roboto, sans-serif'),
            fontWeight: 600,
